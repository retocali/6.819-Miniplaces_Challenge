...
def vggnet(x, keep_dropout, train_phase):
	...
	weights = {
	    ...
	    'wfc2': tf.get_variable('wfc2', shape=[4096, 4096], initializer=initializer),
            ...
            'wo': tf.get_variable('wo', shape=[4096, 100], initializer=initializer),
	}
	...
	#conv10= tf.nn.relu(conv10)
        pool4 = tf.nn.max_pool(conv9, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
	...
	#conv13= tf.nn.relu(conv13)
        pool5 = tf.nn.max_pool(conv12, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
	...
	#fc3 = tf.add(tf.matmul(fc2, weights['w0']), biases['b0'])
        ...
        # Output
        out = tf.add(tf.matmul(fc1, weights['wo']), biases['bo'])
...
def vgg_net_run(dropout, batch_size, learning_rate, training_iters):
	...
	loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=logits))
        ...
        train_optimizer = tf.train.MomentumOptimizer(learning_rate,0.9).minimize(loss);
